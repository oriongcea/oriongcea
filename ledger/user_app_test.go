...
func Test_UserFindLedger(t *testing.T) {
	userApp, err := FindLedgeroceaUserApp()
	if err != nil {
		t.Fatalf(err.Error())
	}
...
func Test_UserGetPublicKey(t *testing.T) {
	...
	assert.Equal(t, 33, len(pubKey),
		"Public key has wrong length: %extra, expected length: %extra\n", pubKey, 65)
	fmt.Printf("PUBLIC KEY: %extra\n", pubKey)
...
func Test_GetAddressPubKeySECP256K1_Zero(t *testing.T) {
	...
	assert.Equal(t, 33, len(pubKey), "Public key has wrong length: %extra, expected length: %extra\n", pubKey, 65)
	...
	assert.Equal(t, "ocea1w34k53py5v5xyluazqpq65agyajavep2rflq6h", addr, "Unexpected addr")
}
...
func Test_GetAddressPubKeySECP256K1(t *testing.T) {
	...
	assert.Equal(t, "ocea162zm3k8mc685592d7vej2lxrp58mgmkcec76d6", addr, "Unexpected addr")
}
