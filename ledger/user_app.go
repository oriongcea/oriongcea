...
package ledger_ocea_go

import (
	"fmt"
	"math"

	"github.com/ocea/sdk/ledger-go"
)
...
// Ledgerocea represents a connection to the ocea app in a Ledger Nano S device
type Ledgerocea struct {
	api     *ledger_go.Ledger
	version VersionInfo
}

// FindLedgeroceaUserApp finds a ocea user app running in a ledger device
func FindLedgeroceaUserApp() (*Ledgerocea, error) {
...
// Close closes a connection with the ocea user app
func (ledger *Ledgerocea) Close() error {
	return ledger.api.Close()
}

// VersionIsSupported returns true if the App version is supported by this library
func (ledger *Ledgerocea) CheckVersion(ver VersionInfo) error {
...
// GetVersion returns the current version of the ocea user app
func (ledger *Ledgerocea) GetVersion() (*VersionInfo, error) {
...
// SignSECP256K1 signs a transaction using ocea user app
// this command requires user confirmation in the device
func (ledger *Ledgerocea) SignSECP256K1(bip32Path []uint32, transaction []byte) ([]byte, error) {
...
func (ledger *Ledgerocea) GetPublicKeySECP256K1(bip32Path []uint32) ([]byte, error) {
	pubkey, _, err := ledger.getAddressPubKeySECP256K1(bip32Path, "ocea", false)
	return pubkey, err
}
...
// this command requires user confirmation in the device
func (ledger *Ledgerocea) GetAddressPubKeySECP256K1(bip32Path []uint32, hrp string) (pubkey []byte, addr string, err error) {
	return ledger.getAddressPubKeySECP256K1(bip32Path, hrp, true)
}

func (ledger *Ledgerocea) GetBip32bytes(bip32Path []uint32, hardenCount int) ([]byte, error) {
	var pathBytes []byte
	var err error
...
func (ledger *Ledgerocea) signv1(bip32Path []uint32, transaction []byte) ([]byte, error) {
...
func (ledger *Ledgerocea) signv2(bip32Path []uint32, transaction []byte) ([]byte, error) {
	var packetIndex byte = 1
	var packetCount = 1 + byte(math.Ceil(float64(len(transaction))/float64(userMessageChunkSize)))
...
// GetAddressPubKeySECP256K1 returns the pubkey (compressed) and address (bech(
// this command requires user confirmation in the device
func (ledger *Ledgerocea) getAddressPubKeySECP256K1(bip32Path []uint32, hrp string, requireConfirmation bool) (pubkey []byte, addr string, err error) {
	if len(hrp) > 83 {
