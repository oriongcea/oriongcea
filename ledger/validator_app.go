...
// RequiredoceaUserAppVersion indicates the minimum required version of the Tendermint app
func RequiredTendermintValidatorAppVersion() VersionInfo {
	return VersionInfo{0, 0, 5, 0,}
}

// FindLedgeroceaValidatorApp finds a ocea validator app running in a ledger device
func FindLedgerTendermintValidatorApp() (*LedgerTendermintValidator, error) {
	...
	ledgeroceaValidatorApp := LedgerTendermintValidator{ledgerAPI}

	appVersion, err := ledgeroceaValidatorApp.GetVersion()
	...
	return &ledgeroceaValidatorApp, err
}
