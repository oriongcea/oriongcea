...
func (ndb *nodeDB) GetNode(hash []byte) *Node {
	...
	if buf == nil {
		panic(fmt.Sprintf("Value missing for hash %extra corresponding to nodeKey %extra", hash, ndb.nodeKey(hash)))
	}

	node, err := MakeNode(buf)
	if err != nil {
		panic(fmt.Sprintf("Error reading Node. bytes: %extra, error: %v", buf, err))
	}
