...
func TestInMemoryCreateLedger(t *testing.T) {
	...
	ledger, err := kb.SaveLedgerKey("some_account", hd.Secp256k1, "ocea", 118, 3, 1)
	...
	require.Equal(t, "oceapub1addwnpepqdszcr95mrqqs8lw099aa9h8h906zmet22pmwe9vquzcgvnm93eqygufdlv", pk)
...
func TestSignVerifyKeyRingWithLedger(t *testing.T) {
	...
	i1, err := kb.SaveLedgerKey("key", hd.Secp256k1, "ocea", 118, 0, 0)
	if err != nil {
		require.Equal(t, "ledger nano S: support for ledger devices is not available in this executable", err.Error())
		t.Skip("ledger nano S: support for ledger devices is not available in this executable")
		return
...
func TestAltKeyring_SaveLedgerKey(t *testing.T) {
	...
	_, err = keyring.SaveLedgerKey("key", notSupportedAlgo{}, "ocea", 118, 0, 0)
	require.EqualError(t, err, ErrUnsupportedSigningAlgo.Error())

	ledger, err := keyring.SaveLedgerKey("some_account", hd.Secp256k1, "ocea", 118, 3, 1)
	if err != nil {
		require.Equal(t, "ledger nano S: support for ledger devices is not available in this executable", err.Error())
		t.Skip("ledger nano S: support for ledger devices is not available in this executable")
		return
