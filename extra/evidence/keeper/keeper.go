...
func (k Keeper) Logger(ctx sdk.Context) log.Logger {
	return ctx.Logger().With("module", fmt.Sprintf("extra/%s", types.ModuleName))
}

// SetRouter sets the Evidence Handler router for the extra/evidence module. Note,
...
func (k *Keeper) SetRouter(rtr types.Router) {
	...
	if k.router != nil {
		panic(fmt.Sprintf("attempting to reset router on extra/%s", types.ModuleName))
	}
