...
func (s *IntegrationTestSuite) TestTotalSupplyGRPCHandler() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			fmt.Sprintf("%s/ocea/bank/v1beta1/supply", baseURL),
			map[string]string{
		...
		},
		{
			...
			fmt.Sprintf("%s/ocea/bank/v1beta1/supply/%s", baseURL, s.cfg.BondDenom),
			map[string]string{
				grpctypes.GRPCBlockHeightHeader: "1",
			},
		...
		},
		{
			...
			fmt.Sprintf("%s/ocea/bank/v1beta1/supply/%s?height=2", baseURL, s.cfg.BondDenom),
			map[string]string{
				grpctypes.GRPCBlockHeightHeader: "1",
			},
			&types.QuerySupplyOfResponse{},
		...
		},
		{
			...
			fmt.Sprintf("%s/ocea/bank/v1beta1/supply/foobar", baseURL),
			map[string]string{
				grpctypes.GRPCBlockHeightHeader: "1",
			},
			&types.QuerySupplyOfResponse{},
...
func (s *IntegrationTestSuite) TestBalancesGRPCHandler() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			fmt.Sprintf("%s/ocea/bank/v1beta1/balances/%s", baseURL, val.Address.String()),
		...
		},
		{
			...
			fmt.Sprintf("%s/ocea/bank/v1beta1/balances/%s/%s", baseURL, val.Address.String(), s.cfg.BondDenom),
		...
		},
		{
			...
			fmt.Sprintf("%s/ocea/bank/v1beta1/balances/%s/foobar", baseURL, val.Address.String()),
			&types.QueryBalanceResponse{},
			&types.QueryBalanceResponse{
				Balance: &sdk.Coin{
					Denom:  "foobar",
