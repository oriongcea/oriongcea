...
func (s *IntegrationTestSuite) TestGRPCQueries() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			fmt.Sprintf("%s/ocea/slashing/v1beta1/signing_infos", baseURL),
			map[string]string{
		...
		},
		{
			...
			fmt.Sprintf("%s/ocea/slashing/v1beta1/signing_infos/%s", baseURL, consAddr),
			map[string]string{
				grpctypes.GRPCBlockHeightHeader: "1",
			},
			false,
		...
		},
		{
			...
			fmt.Sprintf("%s/ocea/slashing/v1beta1/signing_infos/%s", baseURL, "wrongAddress"),
			map[string]string{},
			true,
			&types.QuerySigningInfoResponse{},
			nil,
		...
		{
			"params",
			fmt.Sprintf("%s/ocea/slashing/v1beta1/params", baseURL),
			map[string]string{},
			false,
